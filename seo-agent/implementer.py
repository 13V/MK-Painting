...
def create_pr(changes, repo_root):
    """
    Commit SEO optimization changes directly to main and push.

    Returns (commit_url, commit_hash) or (None, None) on failure.
    """
    ...
    success = commit_to_main(changed_files, commit_msg, repo_root)
    if success:
...
def commit_to_main(changed_files, commit_msg, repo_root):
    """
    Commit changed files directly to main and push.
    ...
    try:
        ...
        _run_git(repo_root, "pull", "--rebase", "origin", "main")
...
def create_new_page_pr(opportunity, changed_files, repo_root):
    """
    Commit a new landing page directly to main and push.

    Returns (commit_url, commit_hash) or (None, None).
    """
    suburb = opportunity["suburb"].title()
    commit_msg = (
...
def create_blog_pr(opportunity, changed_files, repo_root):
    """
    Commit a new blog article directly to main and push.
